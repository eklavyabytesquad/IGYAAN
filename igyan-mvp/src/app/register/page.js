...
const OPTIONS = [
  {
    href: "/register/institutional-suite",
    label: "Institutional Suite",
    badge: "Institutional Suite  Institutions",
    description: "Register your institution to manage AI copilots, automate operations, and orchestrate campus-wide innovation. For super admins, co-admins, students, and faculty.",
  },
  {
      ...
      <div className="theme-surface relative overflow-hidden rounded-3xl border p-10 shadow-2xl shadow-sky-500/10">
        <div className="flex flex-col gap-10 lg:flex-row lg:items-center lg:justify-between">
          <div className="max-w-xl space-y-5">
            ...
            <h1 className="theme-heading text-3xl font-semibold">Create your iGyanAI account</h1>
            <p className="theme-muted text-sm leading-relaxed">Select the registration portal that matches your needs. Both connect to the same secure platform with role-specific features.</p>
          </div>
          <div className="grid w-full max-w-xl gap-5">
            {OPTIONS.map((option) => (
              <Link key={option.href} href={option.href} className="theme-surface group relative overflow-hidden rounded-2xl border p-6 text-left shadow-lg shadow-sky-500/10 transition-transform hover:-translate-y-1">
                ...
                <p className="text-xs font-semibold uppercase tracking-[0.24em] text-sky-500 dark:text-sky-300">{option.badge}</p>
                <h2 className="theme-heading mt-3 text-xl font-semibold transition-colors group-hover:text-sky-600 dark:group-hover:text-sky-300">{option.label}</h2>
                ...
                <span className="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-sky-500 transition group-hover:translate-x-1 group-hover:text-sky-400">
                  Continue
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="h-4 w-4"><path strokeLinecap="round" strokeLinejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" /></svg>
                </span>
              </Link>
            ))}
          </div>
