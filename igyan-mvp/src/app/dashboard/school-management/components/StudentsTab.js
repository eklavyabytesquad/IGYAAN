...
export default function StudentsTab({ schoolId, session, classes, students, user, onRefresh }) {
...
// Faculty filtering: only show classes assigned via faculty_assignments
useEffect(() => {
const loadClasses = async () => {
if (user?.role === "faculty" && session) {
...
school_id: schoolId, class_id: selectedClass,
student_id: enrollForm.student_id, session_id: session.id,
...
{user?.role === "faculty" ? "Manage students in your assigned classes" : "Assign students to classes for the active session"}
</p>
</div>
{selectedClass && (
...
className={"rounded-xl px-4 py-2.5 text-sm font-medium transition-all " + (selectedClass === c.id
...
{filteredClasses.length === 0 && <p className="text-sm text-zinc-400">{user?.role === "faculty" ? "No classes assigned to you." : "No classes available. Create classes first."}</p>}
</div>
</div>
...
<tr>
<th className={thClass}>Roll</th><th className={thClass}>Student</th>
<th className={thClass}>Email</th><th className={thClass}>Phone</th>
<th className={thClass}>Enrolled On</th><th className={thClass + " text-right"}>Actions</th>
</tr>
</thead>
<tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
{loadingStudents ? (
<tr><td colSpan={6} className={emptyClass}><div className="mx-auto h-8 w-8 animate-spin rounded-full border-3 border-indigo-500 border-t-transparent" /></td></tr>
...
<td className={tdClass + " text-right"}><button onClick={() => handleRemove(cs.id)} className={btnDanger}>Remove</button></td>
</tr>
))}
</tbody>
</table>
