...
export default function AICopilotPage() {
	...
	useEffect(() => {
		if (user) {
			const savedChats = localStorage.getItem(`faculty_chat_history_${user.id}`);
			const savedSessionMemories = localStorage.getItem(`faculty_session_memories_${user.id}`);
			const savedOverallMemories = localStorage.getItem(`faculty_overall_memories_${user.id}`);
			const savedProfile = localStorage.getItem(`faculty_profile_${user.id}`);
			...
			if (savedProfile) {
				setFacultyProfile(JSON.parse(savedProfile));
			} else {
	...
	const saveChat = (chatId, updatedMessages, chatTitle = null) => {
		...
		localStorage.setItem(`faculty_chat_history_${user.id}`, JSON.stringify(updatedChatHistory));
	};
	...
	const saveMemory = (summary, type = "overall") => {
		...
		if (type === "session") {
			...
			localStorage.setItem(`faculty_session_memories_${user.id}`, JSON.stringify(updatedMemories));
		} else {
			const updatedMemories = [newMemory, ...overallMemories];
			setOverallMemories(updatedMemories);
			localStorage.setItem(`faculty_overall_memories_${user.id}`, JSON.stringify(updatedMemories));
		}
	};
	...
	const buildContextPrompt = () => {
		...
		let context = `You are ${facultyProfile.aiName || "Sudarshan AI"}, an AI assistant helping ${facultyProfile.name}`;
		if (facultyProfile.designation) context += `, ${facultyProfile.designation}`;
		if (facultyProfile.department) context += ` from ${facultyProfile.department}`;
		if (facultyProfile.school?.name) context += ` at ${facultyProfile.school.name}`;
		if (facultyProfile.school?.location) context += `, ${facultyProfile.school.location}`;
		context += ". ";
		
		if (facultyProfile.experience) context += `They have ${facultyProfile.experience} years of teaching experience. `;
		if (facultyProfile.subjects?.length > 0) context += `They teach ${facultyProfile.subjects.join(", ")}. `;
		if (facultyProfile.grades?.length > 0) context += `Teaching grades: ${facultyProfile.grades.join(", ")}. `;
		if (facultyProfile.teachingStyle) context += `Teaching style: ${facultyProfile.teachingStyle}. `;
		
		context += `\n\nYou are assisting a teacher/faculty member with teaching preparation, curriculum planning, student assessment, educational resources, and professional development. Provide teacher-focused insights and pedagogical support.`;
		...
		if (selectedNotes) {
			context += `\n\nThe teacher is currently exploring curriculum material: ${selectedNotes.grade}, ${selectedNotes.subject}, ${selectedNotes.chapter}, Topic: ${selectedNotes.topic}. `;
			context += `Please provide teaching resources, lesson planning ideas, assessment strategies, and pedagogical insights relevant to this specific topic and grade level.`;
		}
...
- Keep language simple and relatable for a 9th-grade student`,
	...
	const startNewChat = () => {
		...
		if (user) {
			localStorage.removeItem(`faculty_session_memories_${user.id}`);
		}
	};
	...
	const deleteChat = (chatId) => {
		...
		localStorage.setItem(`faculty_chat_history_${user.id}`, JSON.stringify(updatedChats));
	...
	const handleSaveProfile = (profileData) => {
		...
		localStorage.setItem(`faculty_profile_${user.id}`, JSON.stringify(profileData));
	...
	// Get AI name
	const getAIName = () => {
		return facultyProfile?.aiName || "Sudarshan AI";
	};
					...
					<div className="flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-orange-400 via-yellow-400 to-orange-500 shadow-lg ring-2 ring-orange-200 dark:ring-orange-900/50">
						...
						<div>
							...
							<p className="text-xs" style={{ color: 'var(--dashboard-muted)' }}>
								{facultyProfile?.name ? `Your Teaching Assistant for ${facultyProfile.name}` : 'Powered by iGyan AI'}
							</p>
						</div>
					</div>
